...
export const customers = async (commandEvent) => {
  ...
  try {
    ...
    try {
      ...
      // forEach QueryResponse.Customer
      (_g = qb_customers.QueryResponse.Customer) === null || _g === void 0
        ? void 0
        : _g.forEach(async (cutomer) => {
            var _a, _b, _c;
      ...
      if (result.QuickBooks_total > 1 || result.repzo_total > 1) {
        await commandLog.addDetail(JSON.stringify(result)).commit();
      }
      await commandLog
        .setStatus("success")
